...
export const hrWorkspaceApi = {
  ...
  async createEmployee(payload) {
    return withFallback(
      async () =>
        extractResponseData(await apiClient.post("/v1/hr/employees", payload)),
      async () => {
        const record = {
          id: `emp-${Date.now()}`,
  ...
  async getRoleTemplates() {
    return withFallback(
      async () =>
        extractResponseData(await apiClient.get("/v1/hr/role-templates")),
  ...
  async updateRoleTemplatePermissions(roleName, permissions) {
    return withFallback(
      async () =>
        extractResponseData(
          await apiClient.put(`/v1/hr/role-templates/${roleName}/permissions`, {
            permissions,
          }),
        ),
      async () => {
        ...
        return structuredClone(
          mockState.roleTemplates.find(
            (roleTemplate) => roleTemplate.name === roleName,
          ),
        );
      },
    );
  ...
  async getDepartments() {
    return withFallback(
      async () =>
        extractResponseData(await apiClient.get("/v1/hr/departments")),
  ...
  async getEmployeeProfile(employeeId) {
    return withFallback(
      async () =>
        extractResponseData(
          await apiClient.get(`/v1/hr/employees/${employeeId}/profile`),
        ),
      async () => {
        const fallbackProfile =
          ...
          (() => {
            const employee =
              mockState.employees.find((item) => item.id === employeeId) ??
  ...
  async getPayrollLogs() {
    return withFallback(
      async () =>
        extractResponseData(await apiClient.get("/v1/hr/payroll/logs")),
  ...
  async processPayroll(payload) {
    return withFallback(
      async () =>
        extractResponseData(
          await apiClient.post("/v1/hr/payroll/process", payload),
        ),
      async () => {
        const record = {
          id: `pay-${Date.now()}`,
  ...
  async getWorkflowDefinitions() {
    return withFallback(
      async () =>
        extractResponseData(
          await apiClient.get("/v1/hr/workflows/definitions"),
        ),
      async () =>
        wrapCollection(structuredClone(mockState.workflowDefinitions)),
    );
  },
  async createWorkflowDefinition(payload) {
    return withFallback(
      async () =>
        extractResponseData(
          await apiClient.post("/v1/hr/workflows/definitions", payload),
        ),
      async () => {
        const definition = { id: `wf-${Date.now()}`, ...payload };
        mockState.workflowDefinitions = [
          definition,
          ...mockState.workflowDefinitions,
        ];
  ...
  async getWorkflowInstances() {
    return withFallback(
      async () =>
        extractResponseData(await apiClient.get("/v1/hr/workflows/instances")),
  ...
  async getCandidates() {
    return withFallback(
      async () =>
        extractResponseData(
          await apiClient.get("/v1/hr/recruitment/candidates"),
        ),
      ...
      async () =>
        extractResponseData(
          await apiClient.post(
            `/v1/hr/recruitment/candidates/${candidateId}/stage`,
            { toStage },
          ),
        ),
      async () => {
  ...
  async getPerformanceDashboard() {
    return withFallback(
      async () =>
        extractResponseData(
          await apiClient.get("/v1/hr/performance/dashboard"),
        ),
  ...
  async getPerformanceReviews() {
    return withFallback(
      async () =>
        extractResponseData(await apiClient.get("/v1/hr/performance/reviews")),
  ...
  async getNotifications() {
    return withFallback(
      async () =>
        extractResponseData(await apiClient.get("/v1/hr/notifications")),
  ...
  async getAiInsights() {
    return withFallback(
      async () =>
        extractResponseData(await apiClient.get("/v1/hr/ai/insights")),
